...
import net.mcreator.remeke.gui.RukzakguiGui;
...
public class RukzakItem extends RemekeModElements.ModElement {
	@ObjectHolder("remeke:rukzak")
	public static final Item block = null;
	public RukzakItem(RemekeModElements instance) {
		super(instance, 21);
	...
	public void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == block) {
			if (Minecraft.getInstance().currentScreen instanceof RukzakguiGui.GuiWindow) {
	...
	public static class ItemCustom extends Item {
		public ItemCustom() {
			super(new Item.Properties().group(VkladkaItemGroup.tab).maxStackSize(1).rarity(Rarity.COMMON));
			setRegistryName("rukzak");
		}
			...
			if (entity instanceof ServerPlayerEntity) {
				NetworkHooks.openGui((ServerPlayerEntity) entity, new INamedContainerProvider() {
					...
					public ITextComponent getDisplayName() {
						return new StringTextComponent("аўъчръ");
					}
					...
					public Container createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
						...
						return new RukzakguiGui.GuiContainerMod(id, inventory, packetBuffer);
					}
				}, buf -> {
